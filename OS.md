...
* 文件操作：
    ...
    6. 截断
* 文件打开与关闭
    * 将指定文件属性从外存拷贝到内存打开文件目录表的一个表目当中，并将表目编号(索引)返回给用户。操作系统维护一个open-file table。
    当用户需要对文件操作时，可以通过索引指定文件。当文件不再使用时，操作系统将打开文件表中该记录删除，如果被修改，将文件写回外存。
### 4.1.2 文件逻辑结构
* 无结构文件(流式文件)
* 有结构文件
    1. 顺序文件
...
* 文件控制块和索引节点
    1. 文件控制块FCB
        ...
        * 存取控制信息(权限)
        * 使用信息(修改时间)
    2. 索引节点。一个FCB64字节，一个盘块1KB，最多只能包含16个FCB。另一种方式一个盘块包括64个目录项，一个目录项16字节，14字节文件名，2字节FCB指针
* 目录结构树形
### 4.1.4 文件共享
### 4.1.5 文件保护 访问控制表Access-Control List ACL
